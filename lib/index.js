...
var TwitterTokenStrategy = (function (_OAuthStrategy) {
  ...
  function TwitterTokenStrategy(_options, _verify) {
    ...
    this._oauthTokenField = options.oauthTokenField || 'access_token';
    this._oauthTokenSecretField = options.oauthTokenSecretField || 'access_token_secret';
  ...
  _createClass(TwitterTokenStrategy, [{
    ...
    value: function authenticate(req, options) {
      ...
      var token = (req.body && req.body[this._oauthTokenField]) || (req.query && req.query[this._oauthTokenField]) || (req.headers && req.headers[this._oauthTokenField]);
      var tokenSecret = (req.body && req.body[this._oauthTokenSecretField]) || (req.query && req.query[this._oauthTokenSecretField]) || (req.headers && req.headers[this._oauthTokenSecretField]);
      var userId = (req.body && req.body[this._userIdField]) || (req.query && req.query[this._userIdField]) || (req.headers && req.headers[this._userIdField]) || (token && token.split('-')[0]);
